	...
	namespace {
		...
		public:
			explicit D3D12Device(const DeviceDesc& desc) : desc_(desc) {
				...
				ENGINE_HR(CreateDXGIFactory2(factory_flags, IID_PPV_ARGS(&factory_)));
				...
				// ---- feature asserts: fail LOUDLY at startup, not mysteriously later ----
				{
					...
					ENGINE_HR(device_->CheckFeatureSupport(D3D12_FEATURE_SHADER_MODEL,
						&sm, sizeof(sm)));
					...
					ENGINE_HR(device_->CheckFeatureSupport(D3D12_FEATURE_D3D12_OPTIONS,
						&opts, sizeof(opts)));
				...
				if (desc.enable_debug) {
					...
					if (SUCCEEDED(device_.As(&iq))) {
						...
						log::warn("d3d12: ID3D12InfoQueue1 unavailable; validation "
				...
				ENGINE_HR(device_->CreateCommandQueue(
