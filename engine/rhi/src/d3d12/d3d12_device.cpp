...
namespace engine::rhi {
    namespace {
        ...
        public:
            ...
            // ---- milestone 3 ----
            void set_pso(PSOHandle) override { check(false && "m3"); }
            void set_index_buffer(BufferHandle, Format) override { check(false && "m3"); }
            void push_constants(const void *, uint32_t) override { check(false && "m3"); }
            void draw(uint32_t, uint32_t) override { check(false && "m3"); }
            void draw_indexed(uint32_t, uint32_t) override { check(false && "m3"); }
            void dispatch(uint32_t, uint32_t, uint32_t) override { check(false && "m3"); }
        ...
        public:
            explicit D3D12Device(const DeviceDesc &desc) : desc_(desc) {
                ...
                if (desc.enable_debug) {
                    ...
                    if (SUCCEEDED(D3D12GetDebugInterface(IID_PPV_ARGS(&debug)))) {
                        ...
                        log_warn("d3d12: debug layer unavailable (install the "
                ...
                ENG_HR(CreateDXGIFactory2(ff, IID_PPV_ARGS(&factory_)));
                ...
                check(device_ && "no D3D12 FL12.0 hardware adapter found");
                ...
                ENG_HR(device_->CreateDescriptorHeap(&hd, IID_PPV_ARGS(&rtv_heap_)));
                rtv_size_ = device_->GetDescriptorHandleIncrementSize(
                    D3D12_DESCRIPTOR_HEAP_TYPE_RTV);

                check(desc.native_window && "D3D12 backend needs an HWND");
                create_swapchain();
                acquire_backbuffers();
            }
            ...
            FrameContext begin_frame() override {
                ...
                ENG_HR(slot.allocator->Reset());
                ENG_HR(cmdlist_->Reset(slot.allocator.Get(), nullptr));
            ...
            void end_frame() override {
                ...
                if (pr == DXGI_ERROR_DEVICE_REMOVED || pr == DXGI_ERROR_DEVICE_RESET) {
                    ...
                    log_error("d3d12: DEVICE REMOVED, reason 0x{:08x} — DRED "
            ...
            void resize(uint32_t w, uint32_t h) override {
                ...
                wait_idle();
                release_backbuffers();
                ENG_HR(swapchain_->ResizeBuffers(0, w, h, DXGI_FORMAT_UNKNOWN, 0));
                acquire_backbuffers();
                log_info("d3d12: resized to {}x{}", w, h);
            }
            ...
            void destroy(BufferHandle) override { check(false && "m3"); }
            ...
            void destroy(PSOHandle) override { check(false && "m3"); }
            ...
            void unmap(BufferHandle) override { check(false && "m3"); }
        ...
        private:
            ...
            void assert_features() {
                ...
                ENG_HR(device_->CheckFeatureSupport(D3D12_FEATURE_SHADER_MODEL, &sm,
                                                    sizeof(sm)));
                check(sm.HighestShaderModel >= D3D_SHADER_MODEL_6_6 &&
                      "SM 6.6 required — on the Agility runtime? check exe_dir/D3D12/");

                D3D12_FEATURE_DATA_D3D12_OPTIONS o{};
                ENG_HR(device_->CheckFeatureSupport(D3D12_FEATURE_D3D12_OPTIONS, &o,
                                                    sizeof(o)));
                check(o.ResourceBindingTier == D3D12_RESOURCE_BINDING_TIER_3 &&
                ...
                ENG_HR(device_->CheckFeatureSupport(D3D12_FEATURE_D3D12_OPTIONS12,
                                                    &o12, sizeof(o12)));
                check(o12.EnhancedBarriersSupported && "enhanced barriers required");
            }
            ...
            void create_swapchain() {
                ...
                ENG_HR(factory_->CreateSwapChainForHwnd(queue_.Get(), hwnd, &sd,
                                                        nullptr, nullptr, &sc1));
                ENG_HR(sc1.As(&swapchain_));
                // We own resize; DXGI's alt-enter fullscreen toggle would fight us.
                ENG_HR(factory_->MakeWindowAssociation(hwnd, DXGI_MWA_NO_ALT_ENTER));
            }

            void acquire_backbuffers() {
                for (uint32_t i = 0; i < kBackbufferCount; ++i) {
                    D3D12Texture t;
                    ENG_HR(swapchain_->GetBuffer(i, IID_PPV_ARGS(&t.resource)));
                    ...
                    backbuffers_[i] =
                            to_rhi<TextureHandle>(textures_.create(std::move(t)));
                }
            }
            ...
            static void CALLBACK on_debug_message(D3D12_MESSAGE_CATEGORY,
                ...
                switch (severity) {
                    ...
                    case D3D12_MESSAGE_SEVERITY_ERROR:
                        ...
                        check(false && "D3D12 validation error — see log");
                        break;
        ...
        void D3D12CommandList::barrier(std::span<const TextureBarrier> textures,
            ...
            check(buffers.empty() && "buffer barriers arrive in m3");
        ...
        void D3D12CommandList::set_render_targets(std::span<const TextureHandle> colors,
                                                  TextureHandle depth) {
            check(depth.is_null() && "depth arrives in m3");
            check(colors.size() <= kMaxColorTargets);
