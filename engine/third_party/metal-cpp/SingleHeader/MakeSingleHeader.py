...
class HeaderPrefix(object):
    ...
    def __get_commit_hash(self):
        ...
        try:
            ...
            proc = subprocess.Popen(['git', 'rev-parse', 'HEAD'], cwd=git_dir, stdout=subprocess.PIPE,
...
class SingleHeader(object):
    ...
    def __process_include_directives(self, header):
        return re.sub('^\\s*#include\\s\\"(?P<HEADER_PATH>\\S*)\\"', self.__substitute_include_directive, header,
                      flags=re.MULTILINE)
    ...
    def __process_foundation_directives(self, header):
        if header.find("#include <Foundation/Foundation.hpp>") != -1:
            ...
            return header.replace("#include <Foundation/Foundation.hpp>", self.__process_header(
...
def create_argument_parser():
    ...
    parser.add_argument('-o', '--output', dest='output_path', metavar='PATH', default=output_path,
