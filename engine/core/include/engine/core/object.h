...
public:                                                                                                                \
    ...
    const ::engine::object::ClassId *get_class() const override { return static_class(); }                             \
                                                                                                                       \
private:
    ...
    template <std::derived_from<Object> T> [[nodiscard]] bool is_a(const Object *o)
    {
        if (!o) return false;
        for (const ClassId *c = o->get_class(); c; c = c->parent)
            if (c == T::static_class()) return true;
        return false;
    }

    // Checked downcast: nullptr when the dynamic type doesn't match.
    // The engine's dynamic_cast replacement (built with /GR- in mind).
    template <std::derived_from<Object> T> [[nodiscard]] T *cast(Object *o)
    { return is_a<T>(o) ? static_cast<T *>(o) : nullptr; }

    template <std::derived_from<Object> T> [[nodiscard]] const T *cast(const Object *o)
