...
namespace engine::arena
{
    ...
    public:
        ...
        explicit Arena(const size_t capacity)
            : base_(std::make_unique_for_overwrite<std::byte[]>(capacity)), capacity_(capacity)
        {
        }
        ...
        template <class T> [[nodiscard]] std::span<T> push_array(const size_t count)
        {
        ...
        template <class T> [[nodiscard]] std::span<T> push_array_uninit(size_t count)
        {
